...
chat.on('connection', function(conn) {
    ...
    conn.on('data', function(message) {
        ...
        if (rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            try {
                ...
                if (data.type == 'pm') console.log('message', chalk.underline(clients[conn.id].un) + ' to ' + chalk.underline(data.extra) + ': ' + data.message)
                else console.log('message', '[' + data.type.charAt(0).toUpperCase() + data.type.substring(1) + '] ' + chalk.underline(clients[conn.id].un) + ': ' + data.message)
    ...
    conn.on('close', function() {
        console.log('socket', chalk.underline(conn.id) + ': disconnected (' + clients[conn.id].ip + ')')
